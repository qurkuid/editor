...
Body contributions use persistent vertex, half-edge, and face IDs. A face anchor stores normalized local `u` and `v` coordinates against the face frame, so rigid movement, Y rotation, and uniform scaling resolve the anchor from the transformed topology without rewriting the measurement node or adding a second history entry.
