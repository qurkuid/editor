...
import { BadgeDollarSign, Bot, Check, LoaderCircle, Star } from 'lucide-react'
...
type RawPainterProductCardProps = {
  ...
  readonly onToggleFavorite?: (product: RawPainterProduct) => void
  readonly onAiRequest?: (product: RawPainterProduct) => void
}
...
export function RawPainterProductCard({
  ...
  return (
    <div className="group relative">
          ...
          {processing ? (
            <span className="absolute inset-0 flex flex-col items-center justify-center gap-1 bg-black/45 text-white backdrop-blur-[1px]">
              ...
              <span className="text-[9px] font-medium">
                {t('rawpainter.card.processingSeamless')}
              </span>
            </span>
          ) : selected ? (
      ...
      {onToggleFavorite ? (
        <button
          aria-label={favorite ? t('painting.favorites.remove') : t('painting.favorites.add')}
          aria-pressed={favorite}
          className={`absolute top-2 left-2 flex h-6 w-6 items-center justify-center rounded-full shadow-sm backdrop-blur transition-colors ${
            favorite ? 'bg-amber-400 text-amber-950' : 'bg-black/45 text-white hover:bg-black/65'
          }`}
          onClick={() => onToggleFavorite(product)}
          title={favorite ? t('painting.favorites.remove') : t('painting.favorites.add')}
          type="button"
        >
      ...
      {onAiRequest ? (
        <button
          ...
          className="absolute right-2 bottom-2 flex h-6 w-6 items-center justify-center rounded-full bg-emerald-600/90 text-white shadow-sm backdrop-blur transition-colors hover:bg-emerald-500 disabled:opacity-50"
          disabled={processing}
          onClick={() => onAiRequest(product)}
