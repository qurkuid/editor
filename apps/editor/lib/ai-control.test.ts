...
describe('AI modeling control plane', () => {
  ...
  test('pushes a body face by an exact metric distance as one undo step', () => {
    ...
    expect(updated).toMatchObject(canonical.body)
  ...
  test('offsets a body face through the canonical executor as one undo step', () => {
    ...
    const result = applyAiModelingPlan({
      message: 'Inset the selected face by 120 mm.',
      patches: [{ op: 'offsetBodyFace', id: body.id, faceId: 'face:0', distance: -0.12 }],
    })
    ...
    expect(updated).toMatchObject(canonical.body)
    expect(updated.revision).toBe(body.revision + 1)
    expect(offsetBodyFace(body, 'face:0', -0.12).body).toEqual(canonical.body)
    expect(useScene.temporal.getState().pastStates).toHaveLength(1)
